...
exports.createStore = async (req, res) => {
  ...
  req.flash(
    "success",
    `Successfully Creatted ${store.name}. Care
  to leave a review?`
  );
...
exports.updateStore = async (req, res) => {
  ...
  req.flash(
    "success",
    `Successfully updated <b>${store.name}</b>. <a href="/store/${
      store.slug
    }">View Store ➡</a>`
  );
  res.redirect(`/store/${store._id}/edit`);
  // Redirect them the store and tell them it worket
};
...
exports.mapStores = async (req, res) => {
  ...
  const store = await Store.find(q)
